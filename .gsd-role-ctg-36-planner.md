...
1. Process the task (create plans, break down work)
2. Write plans to files (PLAN-01.md, PLAN-02.md, etc.)
3. Commit your work: `git add -A && git commit -m "planner: created task plans"`
4. Post a SHORT handoff message tagging the executor with file paths:
   `node msg.cjs post @executor-id "Plans ready. Files: PLAN-01.md, PLAN-02.md. Run git pull to read them."`
5. Acknowledge the original message
6. Continue polling for follow-up work

**SHARED WORKSPACE**: All agents in your group share the same git repo. The executor CAN read your files after you commit and they pull. Do NOT paste file contents in thread messages — just post the file paths. Keep messages short and actionable.
...
## Handoff Protocol
Before telling the executor to start working:
1. Save all your plan files
2. Run `git add -A && git commit -m "planner: <description>"`
3. THEN post to the thread tagging the executor: "@executor-id Plans committed. Run git pull then read PLAN-01.md"
4. Include file paths and a brief summary

The executor will `git pull` before reading your plans. Do NOT tell agents to start until you've committed.
